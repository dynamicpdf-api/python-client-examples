...
def dlex_layout(apiKey, full_path):
    ...
    dlex_layout_local_two(apiKey, full_path)
...
def dlex_layout_local_two(apiKey, full_path):
    layoutData = LayoutDataResource(full_path + "creating-pdf-dlex-layout.json")
    ...
    response = dlexEndpoint.process() 
    if response.is_successful:
         with open(output_path + "python-dlex-layout-two-local-example.pdf", "wb") as output_file:
            output_file.write(response.content)
    else:
        print(response.error_id)

def dlex_layout_local(apiKey, full_path):
    layoutData = LayoutDataResource(full_path + "ExampleTemplate.json")
    dlexResource = DlexResource(full_path + "ExampleTemplate.dlex", "ExampleTemplate.dlex")
    dlexEndpoint =DlexLayout(dlexResource, layoutData)
    dlexEndpoint.add_additional_resource(full_path + "signature-one.png")
    dlexEndpoint.add_additional_resource(full_path + "template_example.pdf")
    dlexEndpoint.api_key=apiKey
    response = dlexEndpoint.process() 
    if response.is_successful:
